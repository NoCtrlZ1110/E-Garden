...
import 'package:e_garden/core/services/study/vocabulary.service.dart';
import 'package:flutter/material.dart';

class LearnModel extends ChangeNotifier {
  ListVocabulary _words;

  ListVocabulary get words => _words;
  set words(ListVocabulary value) {
    _words = value;
    notifyListeners();
  }
  Future<ListVocabulary> fetchListVocab({int bookId, int unitId}) async {
    _words = await VocabularyService.fetchUnitVocab({"bookId": bookId, "unitId": unitId});
    print(_words);
    return _words;
  }

  List<String> _meaningWords;
  ...
  void increase(index, wordsLength) {
  ...
  void decrease(index, wordsLength) {
    if (index > 0) {
      index--;
    } else
      index = wordsLength;
